---
title: "What `IF` and `Condition`al-bert Puns Never end?"
---
...
In [part 2](../2)'s Google Script Editor, we copy and pasted a large chunk of code without going into much detail about it. Let's now take sometime to analyze it.

```js title="Google's Script Editor" showLineNumbers
function myFunction() {
...
4. `if` checks for a conditional statement

```js title="Google's Script Editor" showLineNumbers {10,16,17,20,23}
function myFunction() {
    let sheet = SpreadsheetApp.getActiveSheet();
...
[Please see lab 3 if you need a refresher on this!](../week3/3#dot-notation-vs-bracket-notation)
...
This statement wouldn't work since `hello` is **`defined`**, but if we change the `if` evaluation variable insde the `()` to `goodbye`, it will evaluate as true since `goodbye` is **`undefined`**:
...
But, you can **CHANGE** the Google Sheet if you want your `keys` to be easier to work with.
:::
...
<details>
<summary><b>What happened to the pop-ups in the solution above?</b></summary>
...
<img src="/26SU-ASIAAM-191A/labs/week4/media/putlocation.png" alt="Fill out survey location" style="max-width:500px" />

Then you should magically see a point appear!

<img src="/26SU-ASIAAM-191A/labs/week4/media/showup.png" alt="Point appears on map" style="max-width:500px" />

Now you are ready for the [lab assignment](../../../assignments/week4/lab_assignment/)!

<LabNav prev={{ href: "/labs/week4/3/", label: "Connecting to our Google Sheet with papa parse" }} next={{ href: "/labs/week4/5/", label: "Final Template Code" }} />
