...
<img src="/26SU-ASIAAM-191A/labs/week6/media/newbranchvs.png" alt="The branch button in the VS Code status bar" />

You can then create a name for it:

<img src="/26SU-ASIAAM-191A/labs/week6/media/newbranchname.png" alt="VS Code prompting for a new branch name" />
...
You only need to run it when the branch DOES NOT exist on GitHub!!! After the branch is on GitHub, you only need to use `git push`:
...
What happens when a `git push` affects a file that was changed locally but someone else edited on GitHub?
...
A `merge conflict` occurs when one file was changed in two places. For example, Person A edits line 1 of `readme.md` and Person B also edits line 1 of `readme.md`. Git doesn't know which changes to keep, so a person needs to take a look and manually `merge` them.
...
```bash
error: failed to push some refs to 'https://github.com/albertkun/26SU-ASIAAM-191A-Git-Practicing.git'
...
<img src="/26SU-ASIAAM-191A/labs/week6/media/mergeconflictvscode.png" alt="VS Code merge conflict view with Accept Current Change and Accept Incoming Change options" />

After choosing an option, you can push as normal:
...
<LabNav prev={{ href: "/labs/week6/1/", label: "Cloning a new repository" }} next={{ href: "/labs/week6/3/", label: "Adding a new MapLibreGL plugin" }} />
